...
class Layers(object):
    ...
    def conv(self, x, kernel_size, n_out, strides=1, padding="SAME"):
        ...
        with tf.variable_scope("conv"):
            ...
            W = self.get_kernel(
                'W', [kernel_size, kernel_size, kernel_size, n_in, n_out])
            return tf.nn.conv3d(x, W, (1, strides, strides, strides, 1),
                                padding)
    ...
    def Gconv(self,
        ...
        with tf.variable_scope('Gconv'):
            ...
            W = self.get_kernel("W", [
                kernel_size, kernel_size, kernel_size, xsh[4] * xsh[5] * n_out
            ])
            ...
            if xsh[-1] == 1:
                ...
                WN = tf.reshape(WN, [
                    kernel_size, kernel_size, kernel_size, xsh[4],
                    self.group_dim, n_out, self.group_dim
                ])
                ...
                WN = tf.reshape(WN, [
                    ...
                    xsh[4] * self.group_dim, n_out * self.group_dim
                ])

            # Convolve
            # Gaussian dropout on the weights
            WN *= (1 + drop_sigma * tf.to_float(is_training) *
                   tf.random_normal(WN.get_shape()))
            ...
            if padding == 'REFLECT':
                ...
                xN = tf.pad(
                    xN, [[0, 0], [pad, pad], [pad, pad], [pad, pad], [0, 0]],
                    mode='REFLECT')
            ...
            y = tf.reshape(
                yN,
                [batch_size, ysh[1], ysh[2], ysh[3], n_out, self.group_dim])
        return y
    ...
    def Gres_block(self,
        ...
        with tf.variable_scope(name):
            ...
            with tf.name_scope("shortcut_connection"):
                ...
                x = tf.reshape(x, tf.concat([xsh[:4], [
                    -1,
                ]], 0))
                x = tf.nn.avg_pool3d(x, xksize, xstrides, "SAME")
                x = tf.reshape(x, tf.concat([ysh[:4], [-1, self.group_dim]],
                                            0))
